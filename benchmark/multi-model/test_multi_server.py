...
class TestMultiServer(unittest.TestCase):
    ...
    def test_single_model(self):
        req_rates = [28]
        for req_rate in req_rates:
            self.run_single_model_test(
                req_rate=req_rate, cv=1, alpha=1, slo=100, duration=60
            )
    ...
    def test_swap_one(self):
        ...
        alpha = 0.1
        self.run_swap_test(
            req_rate=req_rate, cv=cv, alpha=alpha, swap_policy="baseline"
        )
        self.run_swap_test(
            req_rate=req_rate, cv=cv, alpha=alpha, swap_policy="enhanced"
        )
    ...
    def test_all(self):
        ...
        for req_rate in req_rates:
            for alpha in alphas:
                for slo in slos:
                    print(
                        f"**** Start test req_rate={req_rate}, cv={cv}, alpha={alpha} ****"
                    )
                    self.run_swap_test(
                        req_rate=req_rate,
                        cv=cv,
                    ...
                    self.run_collocate_test(
                        req_rate=req_rate, cv=cv, alpha=alpha, slo=slo
                    )
                    self.run_single_model_test(
                        req_rate=req_rate, cv=cv, alpha=alpha, slo=slo
                    )
