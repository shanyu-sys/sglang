...
async def replace_model(model_path: str, tokenizer_path: str, api_url: str) -> None:
    ...
    # replace the /generate to /replace_model
    replace_model_url = api_url.replace("/generate", "/replace_model")
    assert api_url.endswith("/generate")
    timeout = aiohttp.ClientTimeout(total=3 * 3600)
    async with aiohttp.ClientSession(timeout=timeout) as session:
        while True:
            async with session.post(
                replace_model_url, headers=headers, json=pload
            ) as response:
                chunks = []
                async for chunk, _ in response.content.iter_chunks():
                    chunks.append(chunk)
    ...
    print(f"Replace model latency: {replace_latency:.2f} s")
...
async def benchmark(
    ...
    async for request in get_request(input_requests, request_rate):
        ...
        if counter == 32:
            ...
            await replace_model(model_path, tokenizer_path, api_url)
